...
def build_subdiv_interp_matrix(template, n_subdiv):
    """
    Args:
        template: Base template mesh
        n_subdiv: Number of mesh subdivisions

    Return:
        M: (N_subdiv, N_orig) matrix with interpolation coefficients.

    """
    x = torch.empty(len(template.verts_packed()), dtype=torch.float32,
                    device=template.device, requires_grad=True)

    subdivide = SubdivideMeshes()
    template_i = template
    x_i = x
    for i in range(n_subdiv):
        template_i, x_i = subdivide(template_i, feats=x_i)

    M = []
    for xx in x_i:
        if x.grad is not None:
            x.grad.zero_()
        xx.backward(retain_graph=True)
        M.append(x.grad.clone())

    return torch.vstack(M)

        
class LocalMeshDecoderTrainer:
    @classmethod
    def add_argparse_args(cls, parent_parser):
        ...
        parser.add_argument('--lv_template_subdiv', type=int, default=0)
    ...
    def __init__(self, device=None, log_wandb=True, **kwargs):
        ...
        self.decoder = MeshDecoder(
            ...
            norm=self.normalization[0],
        )
        self.latent_vectors = torch.tensor([])  # Dummy value

        lv_template = pytorch3d.utils.ico_sphere(self.lv_template_subdiv)
        self.num_latent_vectors = len(lv_template.verts_packed())
        self.vert_interp_matrix = build_subdiv_interp_matrix(
            lv_template,
            self.template_subdiv - self.lv_template_subdiv
        )
    ...
    def to(self, device):
        ...
        self.vert_interp_matrix = self.vert_interp_matrix.to(device)
        return self
    ...
    def train(self, train_meshes, val_meshes):
        ...
        self.latent_vectors = nn.Embedding(
            num_train_samples,
            self.num_latent_vectors * self.latent_features,
            max_norm=1.0,
            device=self.device,
        )
    ...
    def prepare_batch(self, batch_idxs):
        ...
        batch_latent_vectors = batch_latent_vectors.view(
            -1, self.num_latent_vectors, self.latent_features)
        batch_latent_vectors = self.vert_interp_matrix @ batch_latent_vectors
        batch_latent_vectors = batch_latent_vectors.view(-1, self.latent_features)
